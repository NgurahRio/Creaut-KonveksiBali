    ...
    <Container className="pt-10 lg:pt-20">
      <section className="grid gap-12 md:grid-cols-2 lg:grid-cols-[0.85fr_1.15fr] lg:items-center">
        <div className="grid gap-5 sm:grid-cols-[150px_1fr]">
          <div className="grid grid-cols-3 gap-4 sm:grid-cols-1">
            {product.detailImages.slice(1, 4).map((image) => (
              <div
                className="relative aspect-square overflow-hidden rounded-lg bg-white shadow-[0_8px_22px_rgba(0,0,0,0.08)]"
                key={image.src}
              >
                <Image alt={image.alt} className="object-cover" fill sizes="150px" src={image.src} />
              </div>
            ))}
          </div>
        ...
        <div>
          ...
          <p className="mt-8 max-w-5xl text-lg font-semibold leading-9 text-[#4d4d4d] sm:text-xl">
            {product.description}
          </p>
              ...
              <div className="flex flex-wrap gap-8">
                {product.printOptions.map((option, index) => (
                  <div className="text-center" key={option}>
                    <div className="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-[#eadfD4] text-[#a17140]">
          ...
          <a
            className="mt-10 inline-flex h-16 items-center gap-3 rounded-lg bg-[#a17140] px-9 text-base font-black uppercase text-white transition hover:bg-[#875d33]"
